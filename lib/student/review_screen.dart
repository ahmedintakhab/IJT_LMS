...
class ReviewPage extends StatefulWidget {
  ...
  const ReviewPage({Key? key, required this.reviewsData, required this.courseId}) : super(key: key);
...
class _ReviewPageState extends State<ReviewPage> {
  ...
  Widget build(BuildContext context) {
    ...
    print("Review Data on Review page: ${widget.reviewsData}");
    return GetBuilder(
      init: HomeController(),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                  ...
                  Column(
                    ...
                    children: [
                      RatingRowWidget(
                        initialRating: 5,
                        itemCount: 5,
                        percent: (widget.reviewsData['five_star_percentage'] ?? 0) / 100,
                      ),
                      SizedBox(height: 10.h),
                      RatingRowWidget(
                        initialRating: 4,
                        itemCount: 4,
                        percent: (widget.reviewsData['four_star_percentage'] ?? 0) / 100,
                      ),
                      SizedBox(height: 10.h),
                      RatingRowWidget(
                        initialRating: 3,
                        itemCount: 3,
                        percent: (widget.reviewsData['three_star_percentage'] ?? 0) / 100,
                      ),
                      SizedBox(height: 10.h),
                      RatingRowWidget(
                        initialRating: 2,
                        itemCount: 2,
                        percent: (widget.reviewsData['two_star_percentage'] ?? 0) / 100,
                      ),
                      SizedBox(height: 10.h),
                      RatingRowWidget(
                        initialRating: 1,
                        itemCount: 1,
                        percent: (widget.reviewsData['first_star_percentage'] ?? 0) / 100,
                      ),
                    ],
                  )
                ],
              ...
              Align(
                  child: Text(
                    '${widget.reviewsData['total_user_reviews']} Reviews',
                    style: TextStyle(
                        fontFamily: 'Gilroy',
                        fontSize: 14.sp,
                        color: Color(0XFF000000),
              ...
              ListView.builder(
                ...
                itemCount: widget.reviewsData['user_reviews']?.length ?? 0,
                itemBuilder: (context, index) {
                  final userReviews = widget.reviewsData['user_reviews'];
