...
class CategoryWiseCourses extends StatefulWidget {
  ...
  const CategoryWiseCourses({Key? key, required this.slug, required this.categoryName})
      : super(key: key);
  @override
  ...
  Future<void> fetchTrendingCourses() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true && data['data']['courses']['data'] != null) {
          setState(() {
      ...
      print(' Fetch Category wise  courses api error: ${e.toString()}');
      setState(() {
        isLoading = false;
        hasError = true;
      });
